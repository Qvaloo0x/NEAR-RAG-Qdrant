...
def init_clients():
    ...
    return qdrant_client, model, openai_client

qdrant_client, model, openai_client = init_clients()
...
def get_near_price_usd():
    ...
    print(f"🔑 KEY detectada: {CMC_API_KEY[:10]}...")
...
def generate_response(query, context):
    ...
    Responde de forma clara y precisa usando SOLO la información del contexto. Si no sabes algo, di que no está en el contexto.
    """
...
def parse_swap_command(query):
    ...
    if "swap" in query_lower and "for" in query_lower:
        try:
            ...
            token_from = amount_str.split()[1] if len(amount_str.split()) > 1 else "usdc"
            ...
            if token_from == "usdc":
                ...
                return f"✅ Swap: {amount} USDC → {near_amount:.6f} NEAR (${near_price:.4f}/NEAR)"
            else:
                return f"❌ Solo swaps desde USDC por ahora."
        except:
            return "❌ Error parseando swap."
    return None

# Streamlit UI
st.title("🤖 NEAR RAG Assistant")
st.write("Pregunta sobre NEAR Protocol o haz swaps: `swap 1 usdc for near`")
...
# Chat input
if prompt := st.chat_input("Escribe tu pregunta sobre NEAR..."):
    ...
    if swap_result:
        ...
        # RAG normal
        with st.chat_message("assistant"):
            with st.spinner("Buscando en docs de NEAR..."):
                # Buscar contexto
                ...
                # Mostrar contexto usado (opcional)
